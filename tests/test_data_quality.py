"""
Unit tests for the data quality framework. Uses a local SparkSession so the
suite runs in CI without any Databricks/Azure dependency.
"""
...
def test_referential_integrity_flags_orphans(orders_df, customers_df):
    ...
    # customer_id is null for one row, which should not match and is excluded by the join,
    # so orphans should be 0 for the remaining populated keys (101,102,103 all exist).
    assert result.metric_value == 0
    assert result.passed is True
